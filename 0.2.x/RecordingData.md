...
**Quick tip**: There's currently a small bug that stops the hand selection rays from working correctly unless your hands are visible to the tracking cameras when you fire up the scene. Just make sure to hold them in front of you when you press play!
...
This process will take significantly longer, in the realm of 15-20 minutes depending on how many samples you uploaded. You should get a progress bar in the Unity background task monitor, and once it completes you should have two new model files in Assets/StreamingAssets (dev and release). If the progress bar stays at 0 for a while when you start training, that's likely just the GPUs spinning up on the server. Don't worry, they'll get there!
